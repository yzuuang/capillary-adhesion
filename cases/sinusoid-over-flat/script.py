...
def main():
    ...
    h1 = wave_amp * np.cos(qx * xm) * np.cos(qy * ym)

    # # add a bit height variation
    # variation_percent = 1e-2
    # convexity = variation_percent / (0.5*L)**2
    # h1 *= 1 + convexity * (xm - 0.5*L)**2 * (ym - 0.5*L)**2
